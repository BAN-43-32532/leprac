...
int main(int argc, char** argv) {
  ...
  auto fnWow64 = reinterpret_cast<LPFN_ISWOW64PROCESS>(
  ...
  if (isCurrentWow64 == FALSE && isTargetWow64 == TRUE) {
    std::cerr
      << "Bitness mismatch: injector is 64-bit but target process is 32-bit."
      << std::endl;
    std::cerr
      << "Please compile injector as 32-bit for this target."
      << std::endl;
  ...
  HANDLE hThread = CreateRemoteThread(
    hProcess, nullptr, 0, loadLibAddr, remoteMem, 0, nullptr
  );
  if (!hThread) {
  ...
  if (remoteResult == 0) {
    ...
    std::cout
      << "Injection succeeded, module base: 0x"
      << std::hex
      << remoteResult
      << std::dec
      << std::endl;
